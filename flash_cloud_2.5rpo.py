...
#     st.markdown("</div>", unsafe_allow_html=True)
# app.py / flash_cloud_2.5rpo.py
#!/usr/bin/env python3
# -*- coding: utf-8 -*-

import os, time, sqlite3, threading, json, base64, hmac, hashlib, io, csv
from typing import List, Dict, Any, Optional, Tuple, Generator
from urllib.parse import urlparse, quote
...
# ---------- Vertex GenAI (new SDK) ----------
# NOTE: using google-genai (Vertex SDK), NOT google-generativeai
...
def get_genai_client():
    """
    Returns a google.genai.Client configured for Vertex AI in this project/location.
    We intentionally avoid a return type annotation like `-> genai.Client` to
    prevent runtime import issues on some environments.
    """
    creds, label = load_gcp_credentials()
    st.session_state["auth_label"] = label
...
def _ensure_conversation_api():
    global CHAT_DB
    missing = [m for m in ("list_sessions", "rename_session", "all_messages") if not hasattr(CHAT_DB, m)]
    if missing:
        try:
            get_chat_store.clear()
        except Exception:
...
def render_chat_composer() -> Tuple[Optional[str], List[Dict[str, Any]]]:
    ...
    with st.form("chat_composer_form", clear_on_submit=True):
        ...
        with c_plus:
                ...
                with tab_aud:
                    if audio_recorder is not None:
                        ...
                        aud_up = st.file_uploader(
                            "Upload audio", type=["wav", "mp3", "m4a", "webm"], accept_multiple_files=False, key="aud_up"
                        )
                        if aud_up is not None:
                            st.session_state["audio_bytes"] = aud_up.getvalue()
            ...
            if cnt:
                st.markdown(
                    f'<span class="chip">{cnt} attachment{"s" if cnt>1 else ""}</span>',
                    unsafe_allow_html=True,
                )
        ...
        with c_input:
            message = st.text_input(
                "Ask anything",
                placeholder="Ask anything related to WYDOT",
    ...
    if current_user_id() > 0:
        ...
        with signup_tab:
            with st.form("signup_form"):
                se = st.text_input("Email", placeholder="you@company.com", key="s_email")
                sd = st.text_input("Display name (optional)", key="s_display")
                sp = st.text_input("Password", type="password", key="s_pass")
                sc = st.text_input("Confirm password", type="password", key="s_conf")
