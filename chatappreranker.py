...
async def on_chat_resume(thread: Dict):
    ...
    await cl.Message(content="💬 **Conversation resumed!** Continue where you left off.", author="WYDOT Assistant").send()
...
async def on_audio_end():
    ...
    msg = cl.Message(content=f"🎤 Transcribing... ({chunk_count} chunks, {total_bytes} bytes)", author="WYDOT Assistant")
    await msg.send()
...
# Initialize Ranker (Tiny model, fast CPU)
# Use global singleton pattern since cl.cache_resource doesn't exist
...
RETRIEVAL_K = int(os.getenv("RETRIEVAL_K", "10")) # Final K after reranking
...
def search_graph(query: str, index_name: str, use_gemini: bool = False) -> Tuple[str, List[Dict]]:
    ...
    if not ret: return "", []
    try:
        # 1. Fetch more candidates (FETCH_K)
        ...
        if docs:
            ...
            # 3. Select Top K (RETRIEVAL_K)
            top_results = results[:RETRIEVAL_K]
            
            # Reconstruct docs from top results
            reranked_docs = []
            for res in top_results:
                original_doc = docs[res['id']]
                # Add reranking score to metadata for debugging if needed
                original_doc.metadata["score"] = res['score']
...
async def main(message: cl.Message):
    ...
    msg = cl.Message(content="", author="WYDOT Assistant")
    
    # 1. Handle Multimodal (Gemini required)
    if has_media:
        if model_choice != "Gemini 2.5 Flash":
            await cl.Message(content="⚠️ **Please switch to 'Gemini 2.5 Flash' in settings to analyze files.**", author="WYDOT Assistant").send()
            return
