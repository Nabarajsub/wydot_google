...
class WydotDataLayer(BaseDataLayer):
    ...
    async def create_step(self, step_dict):
        ...
        try:
            ...
            if step_id and parent_id and parent_id in _feedback_context:
                # Copy context from msg.id (parent) to step_id so upsert_feedback finds it
                ...
                # Also persist to DB under the step ID
                ...
                # Some steps have no parentId — check if step output matches a recent context
                # by saving step_id as an alias for the most recent feedback context entry
                ...
                if output and _feedback_context:
                    # Find a context whose answer starts with the same text
        ...
        try:
            ...
            if step_id and step_id not in _feedback_context:
                if parent_id and parent_id in _feedback_context:
                    ...
                    # Fallback: match by output content
                    output = step_dict.get("output") if isinstance(step_dict, dict) else getattr(step_dict, "output", None)
                    if output and _feedback_context:
                        for mid, ctx in reversed(list(_feedback_context.items())):
                            if ctx.get("answer") and output[:100] in ctx["answer"][:200]:
    ...
    async def upsert_feedback(self, feedback):
        ...
        try:
            ...
            # This handles edge cases where step ID mapping didn't work
            if not question and _feedback_context:
                ...
                logger.info(f"[FEEDBACK] forId={for_id}, source=fallback_recent, question={question[:50] if question else None}")
