...
# ---- Vertex tuned endpoint (ADC auth) ----
...
def build_pdf_url(source: Optional[str], page: Optional[int]) -> Optional[str]:
    """
    Build a URL to the PDF with a #page=N anchor.
    - If `source` already a URL, use it.
    - Else if PDF_BASE_URL is set, join with basename(source).
    - Else return None.
    """
    if not source:
        return None
...
def render_chat_composer() -> Tuple[Optional[str], List[Dict[str, Any]]]:
    ...
    with st.form("chat_composer_form", clear_on_submit=True):
        ...
        # 🎤 mini popover to add *another* quick audio file if desired
        with c_mic:
            with st.popover("🎤", use_container_width=True):
        ...
        if submitted:
            ...
            if st.session_state.get("audio_bytes"):
                payload.append({
                    ...
                    "mime": "audio/wav",  # recorder returns wav; uploader may vary but fine for model
                    "name": "recording.wav",
                })
            ...
            # clear transient states after submit
            st.session_state["chat_attached_preview"] = []
            st.session_state["camera_image"] = None
            st.session_state["audio_bytes"] = None
            st.session_state["video_file"] = None
...
with st.sidebar:
    ...
    st.caption("Auth: Application Default Credentials (service account or gcloud ADC)")
...
# ===== Layout: single column chat; show retrieved docs right below each answer =====
...
if user_query or uploads_payload:
    ...
    with st.chat_message("assistant"):
        ...
        for partial_text, sources in resultDocuments_streaming(
            ...
            if sources:
                final_sources = sources  # keep latest batch

    # Persist for any other parts of the app (optional)
    st.session_state["last_sources"] = final_sources

    # --- Retrieved documents directly under the assistant reply ---
    st.markdown("#### 📚 Retrieved Documents")
    if not final_sources:
        st.info("No retrieved context for this turn.")
    else:
        # Inline PDF preview region (updates when a user clicks "Inline preview")
        ...
        for i, s in enumerate(final_sources, start=1):
            page = s.get("page")
            source = s.get("source")
            ...
            with st.expander(header, expanded=(i == 1)):
                ...
                # Action row: open link & inline preview
                c1, c2 = st.columns(2)
                with c1:
                    if url:
                        st.link_button(
                ...
                with c2:
                    if url:
                        if st.button("🖼️ Inline preview", key=f"preview_inline_{i}", use_container_width=True):
