...
OAUTH_CLIENT_SECRETS = os.getenv("GOOGLE_OAUTH_CLIENT_SECRET_FILE", "client_secret.json")
# For local dev, this is fine. For deployment, set GOOGLE_OAUTH_REDIRECT_URI to your deployed URL.
OAUTH_REDIRECT_URI = os.getenv("GOOGLE_OAUTH_REDIRECT_URI", "http://localhost:8502")
OAUTH_SCOPES = [
    "openid",
...
# =========================================================
# OAuth token storage (SQLite, per Google account)
# =========================================================
def _get_oauth_conn():
    # Separate small connection; uses same DB file as chat history
...
def save_google_credentials(email: str, creds: OAuthCredentials):
    """Store/overwrite OAuth credentials for a Google account."""
    ...
    with conn:
        conn.execute(
            "REPLACE INTO oauth_tokens (email, credentials) VALUES (?, ?)",
            (email, data),
        )
    conn.close()


def load_google_credentials(email: str) -> Optional[OAuthCredentials]:
    """Load OAuth credentials for a Google account, or None if missing."""
    if not email:
        return None
    email = (email or "").strip().lower()
    conn = _get_oauth_conn()
    try:
        cur = conn.execute(
            "SELECT credentials FROM oauth_tokens WHERE email=?",
            (email,),
        )
        row = cur.fetchone()
    finally:
    ...
    try:
        ...
        return OAuthCredentials.from_authorized_user_info(data, scopes=OAUTH_SCOPES)
    except Exception:
        return None
...
def get_workspace_context(query: str, max_chars: int = 4000) -> str:
    """
    Pull short snippets from Gmail and Drive to use as extra RAG context.

    Priority:
    1. Use per-user OAuth credentials (Google login in sidebar).
    2. Fallback to legacy service-account based access if ENABLE_WORKSPACE_CONTEXT=1.

    If anything fails, returns "" and does not break main chat.
    """
    if not query or not query.strip():
        return ""
    try:
    ...
    # ---------- 1) Try per-user OAuth credentials ----------
    email = None
    guser = st.session_state.get("google_user")
    if guser and guser.get("email"):
        email = guser["email"]
    ...
    creds = None
    if email:
        creds = load_google_credentials(email)

    # ---------- 2) Fallback: service account (legacy) ----------
    if not creds:
        if not os.getenv("ENABLE_WORKSPACE_CONTEXT"):
            # No OAuth tokens and SA workspace context disabled
            return ""
        sa_creds, _ = load_gcp_credentials()
        if not sa_creds:
            return ""
        scopes = [
            "https://www.googleapis.com/auth/gmail.readonly",
...
# --- Handle Google OAuth callback (coming back from Google) ---
...
if "code" in params:
    ...
    try:
        ...
        if email:
            ...
            save_google_credentials(email, creds)
            st.success(f"Connected Google account: {email}")
        else:
            st.warning("Google login succeeded but no email was returned.")
    except Exception as e:
        st.error(f"Google OAuth error: {e}")

    # Clear params so app doesn't re-trigger OAuth repeatedly
    st.experimental_set_query_params()

    # 🔥 CRITICAL: Stop execution here so no other UI loads
...
with st.sidebar:
    ...
    if guser:
        st.success(f"Google: {guser.get('email')}")
        st.write("DEBUG google_user:", st.session_state.get("google_user"))
        import os

        st.write("DEBUG client_secret.json exists:", os.path.exists(OAUTH_CLIENT_SECRETS))
        st.write("DEBUG has oauth tokens:", bool(st.session_state.get("google_oauth_token")))
        if st.button("Disconnect Google", use_container_width=True):
            st.session_state["google_user"] = None
            st.toast("Disconnected Google account.", icon="🔌")
    else:
        if os.path.exists(OAUTH_CLIENT_SECRETS):
            if st.button("Sign in with Google", use_container_width=True):
                try:
                    ...
                    # Present link for user to continue login
                    st.markdown(f"[Click here to continue Google login]({auth_url})")
                    st.info("After granting access, you'll be redirected back here.")
                except Exception as e:
                    st.error(f"Failed to start Google login: {e}")
        else:
            st.caption(
                "To enable Workspace mode with Gmail/Drive, place your "
                "`client_secret.json` next to this app file or set "
...
# 2. Render UI
    ...
    if rbac.has(effective_user_id(), "workspace_reader"):
        guser = st.session_state.get("google_user")
        if not guser:
            st.caption("Connect a Google account in the sidebar to pull Gmail/Drive snippets.")
