...
def evaluate_rag_interaction(
    ...
    """
    Compute a bundle of RAG evaluation metrics WITHOUT using an LLM judge:

    - ROUGE-L between answer and concatenated context
    - Lexical precision/recall-style metrics (Precision@k, Recall@k)
    - Lexical groundedness (answer tokens supported by context)
    - Embedding-based similarities as proxies for:
        * RAGAS answer relevance
        * RAGAS context relevance
        * TruLens groundedness
        * G-EVAL answer quality / groundedness
    """
    ...
    # Lexical groundedness: fraction of answer tokens that appear in context
    try:
        ans_tokens = _tokenize_simple(answer)
    ...
    # --- Embedding-based metrics (RAGAS / TruLens / G-EVAL style proxies) ---
    try:
        ...
        # Map to familiar metric names used in the UI
        ...
        metrics.setdefault(
            "notes",
            "Embedding-based heuristic scores; no LLM-judge / JSON parsing.",
        )
    except Exception as e:
